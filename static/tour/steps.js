...
var VERIFIX_TOUR_STEPS = [
  ...
  },
  {
    ...
    why: 'Перетащите тёмную ШАПКУ карточки подразделения (не отдельного сотрудника) на круг локации — прикрепятся СРАЗУ ВСЕ сотрудники этого подразделения (можно и через корневой узел компании — тащить всю компанию целиком).',
    route: null,
    ...
    confirmBeforeAdvance: function () {
      return document.querySelector('[data-has-unattached-employees]')
        ? 'Не всем сотрудникам прикреплена локация. Это значит, что вы не сможете на 100% быть уверены в том, что данные сотрудники сделали свою отметку на рабочем месте. Уверены, что хотите продолжить?'
        : null;
    },
  },
  {
