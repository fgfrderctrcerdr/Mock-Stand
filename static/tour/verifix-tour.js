...
(function (global) {
  ...
  function renderCoach(step) {
    ...
    // По запросу (invite): после первого успешного действия не навязывать
    // ту же подсказку на каждого следующего сотрудника — молча продолжаем
    // поллинг в фоне (autoAdvance сработает сам, когда check() станет
    // true для ВСЕХ), но ничего не подсвечиваем и не показываем.
    if (step.suppressHighlightIf && step.suppressHighlightIf()) {
      pollActive();
      return;
    }
